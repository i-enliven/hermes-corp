...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        """Non-interactive updates apply safe config migrations before restart."""
...
class TestNodeRuntimeNpmResolution:
    ...
    def test_wsl_update_skips_windows_npm_build_paths(self, mock_args, monkeypatch):
        """A Windows-only npm on WSL must not reach ui-tui builds."""
        from hermes_cli import main as hm
        import hermes_constants
...
class TestUpdateNodeDependencies:
    ...
    install (ui-tui) is safe — apps/desktop is simply never named, so
    ...
    def test_install_names_ui_tui_workspaces(self, _which, mock_popen, tmp_path, monkeypatch):
        """Regression for #43564: install ui-tui directly. apps/desktop
        ...
        assert "--workspace ui-tui" in joined, (
            f"expected ui-tui workspace selector; actual: {calls[0]}"
        )
    ...
    def test_install_includes_workspace_root_to_protect_root_devdependencies(
        ...
        still excludes the unnamed apps/desktop workspace
    ...
    def test_warms_npx_agent_browser_cache_regardless_of_install_result(
        ...
        it's independent of ui-tui dependency state (#43564)."""
        from hermes_cli import main as hm
