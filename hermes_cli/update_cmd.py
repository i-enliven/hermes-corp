...
def _reload_config_modules() -> None:
    ...
    It also reloads ``hermes_cli._subprocess_compat`` so that post-update
    cleanup steps use the freshly-pulled code. Without this, a new symbol added
    to ``_subprocess_compat`` (e.g. ``bounded_probe_run``) is invisible to the
...
def _finish_dashboard_update_cleanup(
    ...
    """No-op cleanup: dashboard/serve processes were removed from the CLI."""
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    if _m()._pytest_owns_live_checkout(_m().PROJECT_ROOT):
        # Sandbox posture: never probe the developer's real venv from tests,
        # and never let a missing venv fail an update-driven by tests.
        return True, ""
