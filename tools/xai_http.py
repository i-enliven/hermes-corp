"""Shared helpers for direct xAI HTTP integrations (restored minimal surface).

The xAI HTTP integration helpers were pruned, but the preserved tools surface
(TTS/STT/search/video/web_tools) still resolves credentials and storage
options through them. Recreated from the documented contract; self-contained
— no import guards, no stubs.
"""
...
def has_xai_credentials() -> bool:
    ...
    hot-path availability scans don't incur disk locks or a network token
    refresh. Resolution order, fast-to-slow:

    1. ``XAI_API_KEY`` env var / profile secret scope (cheapest).
    2. ``~/.hermes/auth.json`` providers.xai-oauth tokens.access_token.
    3. ``credential_pool.xai-oauth`` entries with an access_token.

    Returns False on any exception so a corrupted auth store can't block
    other availability scans.
    """
    try:
...
def _coerce_expires_after(value: Any) -> Optional[int]:
    ...
    Returns int seconds for an expiring file, or None for permanent
    storage (omit expires_after on the wire).
    """
    if value is None:
        return None
    if isinstance(value, str):
...
def _resolve_explicit_xai_base_url(default: str = "https://api.x.ai/v1") -> str:
    """Base URL for the explicit-API-key path (origin-pinned, fail-safe)."""
    override = str(
...
def resolve_xai_http_credentials(
    *,
    ...
    Prefers Hermes-managed xAI OAuth credentials when available, then falls
    back to ``XAI_API_KEY`` resolved via ``hermes_cli.config.get_env_value``.
    Set ``prefer_api_key=True`` to invert that order (API-metered endpoints).
    """
    if prefer_api_key:
